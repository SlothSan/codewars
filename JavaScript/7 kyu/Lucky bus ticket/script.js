...
const isLucky = (ticket) => {
  ...
  let luckyTicket = ticket.match(/[0-9]/g);
  if (luckyTicket === null || luckyTicket.length !== 6) return false;
  return (
    luckyTicket.slice(0, 3).reduce((acc, curr) => acc + +curr, 0) ===
    luckyTicket.slice(3).reduce((acc, curr) => acc + +curr, 0)
  );
};
