...
const distributionOf = (golds) => {
  let minerOneTake = true;
  let minerOneTotal = 0;
  let minerTwoTotal = 0;

  while (golds.length > 0) {
    let left = golds[0];
    let right = golds[golds.length - 1];

    if (left > right || left === right) {
      let left = golds.shift();
      if (minerOneTake) {
        minerOneTotal += left;
      } else {
        minerTwoTotal += left;
    ...
    minerOneTake = !minerOneTake;
  }

  return [minerOneTotal, minerTwoTotal];
};

console.log(distributionOf([10, 1000, 2, 1]));
//[ 12, 1001 ]
