...
const zombieShootout = (zombieCount, range, bullets) => {
  let zombiesShot = 0;
  let distance = range;
  while (zombiesShot < zombieCount) {
    if (bullets >= 0) {
      bullets--;
    }
    distance -= 0.5;
    ...
    zombiesShot++;
    if (distance <= 0 && zombiesShot !== zombieCount) {
      return `You shot ${zombiesShot} zombies before being eaten: overwhelmed.`;
    }
  }
  return `You shot all ${zombieCount} zombies.`;
};
