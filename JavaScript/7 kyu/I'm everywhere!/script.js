...
const i = (word) => {
  const checkVowelCount = (check) => {
    let vowelsCount = check
      .toLowerCase()
      .split("")
      .filter((curr) => ["a", "e", "i", "o", "u"].includes(curr)).length;
    return word.length - vowelsCount > vowelsCount;
  };

  let regex = /^(?![I])[A-Z]/;
  if (word.length < 1 || !regex.test(word) || !checkVowelCount(word)) {
    return "Invalid word";
  } else {
    return "i" + word;
  }
