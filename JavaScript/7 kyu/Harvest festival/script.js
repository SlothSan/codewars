...
const plant = (seed, water, fert, temp) => {
  let flower = seed,
    lengthOfStem = water,
    flowerAmount = fert,
    onlyOneFlower = temp < 20 || temp > 30;
  if (onlyOneFlower) {
    let stem = "-".repeat(lengthOfStem);
    if (lengthOfStem > 1) {
      stem = stem.repeat(lengthOfStem);
    }
    return stem + flower;
  } else {
    let stem = "-".repeat(lengthOfStem) + flower.repeat(flowerAmount);
    return stem.repeat(lengthOfStem);
  }
};
