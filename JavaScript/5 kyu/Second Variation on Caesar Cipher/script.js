...
const encodeStr = (s, shift) => {
  ...
  const prefix = first + move(first, shift);
  const encoded = [...s].map((ch) => move(ch, shift)).join("");
  const full = prefix + encoded;

  const size = Math.ceil(full.length / 5);
  const res = [];

  for (let i = 0; i < 5; i++) {
    const part = full.slice(i * size, (i + 1) * size);
...
const decode = (arr) => {
  ...
  const full = arr.join("");
  const shift = (full.charCodeAt(1) - full.charCodeAt(0) + 26) % 26;
  const encoded = full.slice(2);

  return [...encoded].map((ch) => move(ch, -shift)).join("");
};

console.log(encodeStr("Et tu Brutus?", 2)); //[ 'egG', 'v v', 'w D', 'twv', 'wu?' ]
console.log(decode(["egG", "v v", "w D", "twv", "wu?"])); //Et tu Brutus?
