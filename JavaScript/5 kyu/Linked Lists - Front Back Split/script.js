...
const frontBackSplit = (source, front, back) => {
  if (!source || !front || !back) throw new Error("Arguments cannot be null");
  if (!source.next) throw new Error("Source list must have at least two nodes");
  ...
  back.data = backHead.data;
  back.next = backHead.next;
};
