...
const maxSumDig = (nMax, maxSum) => {
  ...
  for (let num = 1000; num <= nMax; num++) {
    ...
    for (let i = 0; i <= digits.length - 4; i++) {
      if (
        digits
          .slice(i, i + 4)
          .split("")
          .reduce((a, b) => a + parseInt(b), 0) > maxSum
      ) {
        valid = false;
        break;
      }
    ...
    if (valid) validNumbers.push(num);
  }

  const count = validNumbers.length;
  const totalSum = validNumbers.reduce((a, b) => a + b, 0);
  const mean = totalSum / count;
  const closestToMean = validNumbers.reduce((prev, curr) =>
    Math.abs(curr - mean) < Math.abs(prev - mean) ||
    (Math.abs(curr - mean) === Math.abs(prev - mean) && curr < prev)
      ? curr
      : prev
  );

  return [count, closestToMean, totalSum];
};
