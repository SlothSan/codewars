...
const rolldiceSumProb = (sum, dice) => {
  ...
  let combinations = 0;
  let totalCombinations = Math.pow(6, dice);

  const calculateCombinations = (currentSum, remainingDice) => {
    if (remainingDice === 0) {
      if (currentSum === sum) {
        combinations++;
      }
    ...
    for (let i = 1; i <= 6; i++) {
      if (
        currentSum + i + (remainingDice - 1) * 6 >= sum &&
        currentSum + i <= sum
      ) {
        calculateCombinations(currentSum + i, remainingDice - 1);
      }
    }
  };

  calculateCombinations(0, dice);
  return combinations / totalCombinations;
};
