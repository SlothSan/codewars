...
const defaultArguments = (fn, defaults) => {
  const originalArgNames =
    fn._argNames ||
    fn
      .toString()
      .replace(/\/\*.*?\*\//gs, "")
  ...
  const mergedDefaults = { ...(fn._defaults || {}), ...defaults };

  const wrapper = (...args) => {
    const finalArgs = originalArgNames.map((name, i) =>
      i < args.length
        ? args[i]
        : mergedDefaults.hasOwnProperty(name)
        ? mergedDefaults[name]
        : undefined
    );
    return fn(...finalArgs);
  ...
  wrapper._defaults = mergedDefaults;
  wrapper._argNames = originalArgNames;
  return wrapper;
};
