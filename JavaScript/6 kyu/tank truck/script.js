...
*/
const tankvol = (height, diameter, volumeTotal) => {
  return Math.floor(
    ((4 * volumeTotal) / (Math.pow(diameter, 2) * Math.PI)) *
      (Math.pow(diameter / 2, 2) * Math.acos(1 - height / (diameter / 2)) -
        (diameter / 2 - height) * Math.sqrt(height * (diameter - height)))
  );
};
