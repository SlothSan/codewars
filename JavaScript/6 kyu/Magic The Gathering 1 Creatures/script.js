...
const battle = (player1, player2) => {
  ...
  const len = Math.max(player1.length, player2.length);
  ...
  for (let i = 0; i < len; i++) {
    ...
    const c1Survives = p2 < t1;
    const c2Survives = p1 < t2;

    if (c1Survives) survivors1.push(c1);
    if (c2Survives) survivors2.push(c2);
  }
...
console.log(battle(player1, player2)); //{ player1: [ [ 5, 5 ] ], player2: [ [ 1, 2 ], [ 3, 3 ] ] }
