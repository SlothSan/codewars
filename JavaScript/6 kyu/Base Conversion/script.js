...
const convert = (input, source, target) => {
  const decimalValue = input.split("").reduce((acc, char) => {
    const value = source.indexOf(char);
    return acc * source.length + value;
  }, 0);
  ...
  do {
    result = target.charAt(quotient % target.length) + result;
    quotient = Math.floor(quotient / target.length);
  } while (quotient !== 0);
