...
const same = (aArr, bArr) => {
  const normalize = (subArr) => subArr.slice().sort((a, b) => a - b);

  const normalizeArray = (arr) => arr.map(normalize).sort();

  // Normalize and sort both arrays
  const normalizedArr1 = normalizeArray(aArr);
  const normalizedArr2 = normalizeArray(bArr);

  // Compare the normalized and sorted arrays
  ...
  for (let i = 0; i < normalizedArr1.length; i++) {
    if (
      normalizedArr1[i][0] !== normalizedArr2[i][0] ||
      normalizedArr1[i][1] !== normalizedArr2[i][1]
    ) {
      return false;
    }
  }
