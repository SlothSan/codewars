...
const catalog = (s, article) => {
  ...
  s = s
    .split("\n")
    .filter((element) => element.includes(article))
    .map((element) => {
      let itemName = element.match(/(?<=<name>)(.*)(?=<\/name)/g);
      let itemPrice = element.match(/(?<=<prx>)(.*)(?=<\/prx>)/g);
      let itemQty = element.match(/(?<=<qty>)(.*)(?=<\/qty>)/g);
      matches.push(`${itemName} > prx: $${itemPrice} qty: ${itemQty}`);
    });
