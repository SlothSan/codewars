...
const birdCode = (birds) => {
  return birds.map(function (x) {
    let bird = x.toUpperCase().split(/[ -]/);
    switch (bird.length) {
      case 1:
        return bird[0].substr(0, 4);
      case 2:
        return bird[0].substr(0, 2) + bird[1].substr(0, 2);
      case 3:
        return bird[0][0] + bird[1][0] + bird[2].substr(0, 2);
      case 4:
