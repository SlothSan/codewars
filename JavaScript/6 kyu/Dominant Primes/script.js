...
const solve = (a, b) => {
  ...
  const isPrimeNumberFlag = new Uint8Array(b + 1);
  isPrimeNumberFlag.fill(1, 2);
  for (let candidate = 2; candidate * candidate <= b; candidate++) {
    if (isPrimeNumberFlag[candidate]) {
      for (
        let multiple = candidate * candidate;
        multiple <= b;
        multiple += candidate
      ) {
        isPrimeNumberFlag[multiple] = 0;
      }
    }
  }

  let primeSequenceIndex = 0,
    dominantPrimeSum = 0;
  for (let value = 2; value <= b; value++) {
    if (isPrimeNumberFlag[value]) {
      primeSequenceIndex++;
      if (value >= a && isPrimeNumberFlag[primeSequenceIndex])
        dominantPrimeSum += value;
    }
  }
  return dominantPrimeSum;
};
