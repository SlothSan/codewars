...
const highlight = (code) => {
  let [result, arr, vaild] = ["", code.split(""), "0123456789"];
  let temp = arr[0];
  for (let i = 1; i <= arr.length; i++) {
    if (
      arr[i - 1] == arr[i] ||
      (vaild.includes(arr[i - 1]) && vaild.includes(arr[i]))
    ) {
      temp += arr[i];
    } else {
      if (arr[i - 1] == "F") {
        result += `<span style="color: pink">${temp}</span>`;
      } else if (arr[i - 1] == "L") {
        result += `<span style="color: red">${temp}</span>`;
      } else if (arr[i - 1] == "R") {
        result += `<span style="color: green">${temp}</span>`;
      } else if (vaild.includes(arr[i - 1])) {
        result += `<span style="color: orange">${temp}</span>`;
      } else {
        result += `${temp}`;
      }
      temp = arr[i];
