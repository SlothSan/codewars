...
const mean = (town, string) => {
  let data = getData(string).get(town);
  if (!data) return -1;
  return (
    data.reduce((accumator, current) => accumator + current, 0) / data.length
  );
};

const variance = (town, string) => {
  let meanData = mean(town, string);
  if (meanData === -1) return -1;
  let data = getData(string).get(town);
  return (
    data
      .map((value) => (value - meanData) * (value - meanData))
      .reduce((accumator, current) => accumator + current, 0) / data.length
  );
};
