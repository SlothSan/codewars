...
const regexTicTacToeWinChecker = (board) => {
  const horizontal = /^(...){0,2}(O|X)\2\2/;
  const vertical = /^.{0,2}(O|X)..\1..\1/;
  const diagonal = /^(O|X)...\1...\1|^..(O|X).\2.\2/;

  return horizontal.test(board) || vertical.test(board) || diagonal.test(board);
};

console.log(regexTicTacToeWinChecker("X--OOOX-X")); //true
