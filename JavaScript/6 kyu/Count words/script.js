...
const wordCount = (text) => {
  const excludedWords = ["a", "the", "on", "at", "of", "upon", "in", "as"];
  const words = text.toLowerCase().match(/[a-z]+/g);
  return words.filter((word) => !excludedWords.includes(word)).length;
};

console.log(wordCount("Hello there general kenobi")); //4
