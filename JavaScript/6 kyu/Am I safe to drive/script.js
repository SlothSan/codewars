...
const drive = (drinks, finished, drive_time) => {
  let totalUnitsConsumed =
    Math.round(
      (drinks.reduce(
        (total, [strength, vol]) => total + (strength * vol) / 1000,
        0
      ) +
        Number.EPSILON) *
        100
  ...
  const [fHour, fMin] = finished.split(":").map(Number);
  const [dHour, dMin] = drive_time.split(":").map(Number);

  const finishedMinutes = fHour * 60 + fMin;
  let driveMinutes = dHour * 60 + dMin;
  if (finishedMinutes >= driveMinutes) {
    driveMinutes += 24 * 60;
  }

  const hoursPassed = (driveMinutes - finishedMinutes) / 60;

  const remainingUnits = totalUnitsConsumed - hoursPassed;

  const canDrive = remainingUnits < 0;

  return [totalUnitsConsumed, canDrive];
};
