...
function GuessWho(character) {
  ...
  this.possible = this.characteristic.map((c) => c[0]);
}
...
GuessWho.prototype.guess = function (guess) {
  ...
  // name guess
  if (this.possible.includes(guess)) {
    if (guess === this.character) return [`Correct! in ${this.turns} turns`];
    this.possible = this.possible.filter((c) => c !== guess);
    return this.possible;
  }

  const charTraits = this.characteristic.find(
    (c) => c[0] === this.character,
  )[1];
  const hasTrait = charTraits.includes(guess);

  this.possible = this.possible.filter((name) => {
    const traits = this.characteristic.find((c) => c[0] === name)[1];
    return hasTrait ? traits.includes(guess) : !traits.includes(guess);
  });
