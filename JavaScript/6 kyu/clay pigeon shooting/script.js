...
const shoot = (x) => {
  ...
  for (let i = 0; i < scores.length; i++) {
    const [shots, double] = [...scores[i]];
    console.log(shots, double);
    const points = double ? 2 : 1;
    peteScore += shots.P1.replace(/O/g, "").length * points;
    philScore += shots.P2.replace(/O/g, "").length * points;
  }
