...
function solve(files) {
  ...
  const maxCount = Math.max(...Object.values(extensionCount));
  const mostCommonExtensions = Object.keys(extensionCount).filter(
    (extension) => extensionCount[extension] === maxCount
  );
  mostCommonExtensions.sort();

  return mostCommonExtensions.map((extension) => `.${extension}`);
}
