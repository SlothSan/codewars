...
function harvesterRescue(data) {
  const harvesterLocation = data.harvester;
  const wormLocation = data.worm[0];
  const wormSpeed = data.worm[1];
  const carryallLocation = data.carryall[0];
  const carryallSpeed = data.carryall[1];

  const distanceHarvesterWorm = Math.sqrt(
    Math.pow(harvesterLocation[0] - wormLocation[0], 2) +
      Math.pow(harvesterLocation[1] - wormLocation[1], 2)
  );

  const distanceHarvesterCarryall = Math.sqrt(
    Math.pow(harvesterLocation[0] - carryallLocation[0], 2) +
      Math.pow(harvesterLocation[1] - carryallLocation[1], 2)
  );

  const timeWormToHarvester = distanceHarvesterWorm / wormSpeed;

  const timeCarryallToHarvester = distanceHarvesterCarryall / carryallSpeed + 1; // Adding 1 minute for lifting the harvester

  return timeCarryallToHarvester < timeWormToHarvester
