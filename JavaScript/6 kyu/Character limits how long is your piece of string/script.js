...
const charCheck = (text, max, spaces) => {
  text = spaces
    ? text
    : text
        .split("")
        .filter((curr) => curr !== " ")
        .join("");
  return [text.length <= max, text.length <= max ? text : text.slice(0, max)];
};
...
[
    false,
    'I swiftly resolved the situation with base-wide water ration'
  ]
*/
