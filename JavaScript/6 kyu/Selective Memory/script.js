...
const select = (memory) => {
  let memories = memory.split(", ");
  let toRemove = new Set();

  for (let i = 0; i < memories.length; i++) {
    if (memories[i][0] === "!") {
      let name = memories[i].slice(1);
      toRemove.add(name);
      if (i + 1 < memories.length) toRemove.add(memories[i + 1]);
    }
  }

  return memories
    .filter(
      (person) => !toRemove.has(person[0] === "!" ? person.slice(1) : person)
    )
    .join(", ");
};
