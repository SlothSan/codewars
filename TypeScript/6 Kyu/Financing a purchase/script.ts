...
export function amort(rate: number, balance: number, term: number, paymentNo: number): string {
    ...
    const c = balance * (monthlyRate / (1 - Math.pow(1 + monthlyRate, -term)));

    let previousBalance = balance;
    for (let i = 1; i < paymentNo; i++) {
        const interest = previousBalance * monthlyRate;
    ...
    const interest = previousBalance * monthlyRate;
    const principal = c - interest;
    const remainingBalance = previousBalance - principal;

    return `num_payment ${paymentNo} c ${Math.round(c)} princ ${Math.round(principal)} int ${Math.round(interest)} balance ${Math.round(remainingBalance)}`;
}